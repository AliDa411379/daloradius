...
function generate_agent_checkboxes($agent_options, $selected_agents = array()) {
    // Always ensure agent ID 1 is selected and add it to selected_agents if not already there
    if (!in_array('1', $selected_agents)) {
        $selected_agents[] = '1';
    }
    ...
    foreach ($agent_options as $agent_id => $agent_name) {
        ...
        $is_default_agent = ($agent_id == '1'); // Check if this is the default agent
        $checked = $is_selected ? 'checked' : '';
        $disabled = $is_default_agent ? 'disabled' : ''; // Make agent ID 1 read-only
        ...
        // Special styling for default agent
        if ($is_default_agent) {
        ...
        $html .= '<input class="form-check-input agent-checkbox" type="checkbox" name="selected_agents[]" value="' . $agent_id . '" id="agent_' . $agent_id . '" ' . $checked . ' ' . $disabled . '>';
        ...
        if ($is_default_agent) {
            $html .= ' <small class="badge bg-primary ms-1">Default Agent</small>';
        } elseif ($is_selected) {
    ...
    function deselectAllAgents() {
        ...
        checkboxes.forEach(function(checkbox) {
            // Do not deselect the default agent (ID 1)
            if (checkbox.value !== "1") {
    ...
    document.addEventListener("change", function(e) {
        if (e.target.classList.contains("agent-checkbox")) {
            // Prevent unchecking the default agent (ID 1)
